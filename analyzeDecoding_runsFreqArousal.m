...
for iSub=1:length(goodSubs)
    for iRoi=bensonROIs%1:length(roiNames) %length(bensonROIs)
        for ibin=1:nbins
            ...
            for arousal=1:2
                if arousal==1
                    arousalTrials{iSub,arousal} = subBaseStimPupil{iSub}'>subStimPupilBaseMedian(iSub);
                else
                    arousalTrials{iSub,arousal} = subBaseStimPupil{iSub}'<subStimPupilBaseMedian(iSub);
                end
                for ifreq=1:max(freqTrials{goodSubs(iSub),1})
                    subBinFreqStdArousal{iSub,iRoi,ibin,ifreq,arousal} =  subBinStdStim{iSub,iRoi,ibin}(:,stimFreqRuns{iSub}==ifreq & arousalTrials{iSub,arousal},:);%vox,trial
                end
            ...
            for ibin=1:nbins
                ...
                for arousal=1:2
                    ...
                    for irun=1:nruns
                        indFirst = (irun-1)*nfreqs + 1;
                        indLast = irun*nfreqs;
                        trainData =  arousalData(:,[1:indFirst-1 indLast+1:ntrials]);
                        trainLabelsFreq = arousalFreqLabels([1:indFirst-1 indLast+1:ntrials]);
                        testData = arousalData(:,indFirst:indLast);
                        classGuessFreq{iSub,iRoi,ibin,arousal}(indFirst:indLast) = classify(testData', trainData', trainLabelsFreq, 'diagLinear');
                        %
                    ...
                    %                     classAccContrast{featureType,arousal}(iSub,iRoi,ibin,arousal) = sum(arousalContrastLabels==classGuessContrast{iSub,iRoi,ibin})/ntrials;%make sure not to include NaN trials?
                end
            end
        end
    end
...
for arousal=1:2
    [max(squeeze(mean(classAccFreq{featureType,arousal},1))), max(squeeze(mean(classAccFreq{featureType,arousal},1)))]
end
