...
%             end
        ...
        %model each ROI as Procrustes transformation of control ROI
        controlRoiTrials{iSub,iRoi} = subBinNullTrialTseries{iSub,iRoi,nbins};
        for ibin=1:nbins
            [procDist(iSub,iRoi,ibin) procFinal{iSub,iRoi,ibin} procTransform{iSub,iRoi,ibin}] = ...
                procrustes(controlRoiTrials{iSub,iRoi}', subBinNullTrialTseries{iSub,iRoi,ibin}');%transforms the 2nd to fit the 1st
            
            c = procTransform{iSub,iRoi,ibin}.c;
            T = procTransform{iSub,iRoi,ibin}.T;
            b = procTransform{iSub,iRoi,ibin}.b;
            % Z = b*Y*T + c;
            invb = 1/b; %=inv(b);
            invT = inv(T);
            
            subBinNullEst{iSub,iRoi,ibin} = invb*(controlRoiTrials{iSub,iRoi}' - c)*invT;
            subBinNullEst{iSub,iRoi,ibin} = subBinNullEst{iSub,iRoi,ibin}';
...
% 
%             %cross-validation: perform procrustes on N-1 trials, use for
%             %Nth trial, then subtract those estimated trials and compute
%             %variance explained.
%             ntrials = size(subBinNullTrialTseries{iSub,iRoi,ibin},2);
%             for itrial=1:ntrials
%                 trainTrials = [1:itrial-1 itrial+1:ntrials];                
%                 trainData = subBinNullTrialTseries{iSub,iRoi,ibin}(:,trainTrials);
%                [procDist(iSub,iRoi,ibin) procFinal{iSub,iRoi,ibin} trialProcTransform] = ...
%                 procrustes(controlRoiTrials{iSub,iRoi}, subBinNullTrialTseries{iSub,iRoi,ibin});%transforms the 2nd to fit the 1st
% 
%             end
        end
...
mean([controlOrigVar(:) controlResidVar(:)])
...
for iRoi=bensonROIs
   ...
   plot(squeeze(mean(origVar(:,iRoi,:)))')
   hold on
   plot(squeeze(mean(residVar(:,iRoi,:)))')
   
   subplot(rows,cols,iRoi+cols)
   gain = squeeze(mean(residVar(:,iRoi,:)))./squeeze(mean(origVar(:,iRoi,:)));
   plot(gain)
end
