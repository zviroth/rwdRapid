...
for iSub=1:length(goodSubs)
    for ihemi=1:2
        for ibin=1:nbins
            traj{iSub,ihemi,ibin} = subBinNullTrialTseries{iSub,1+ihemi,ibin};%subBinAmpNull{iSub,1+ihemi,ibin}.*exp(subBinPhNull{iSub,1+ihemi,ibin}*1i);
        end
        %DMN & cerebellum
        for iRoi=1:2
            curRoi = bensonROIs(end)+2*(iRoi-1)+ihemi;
            traj{iSub,ihemi,nbins+iRoi} = subNullTrialTseries{iSub,curRoi};%subRoiAmpNull{iSub,curRoi}.*exp(subRoiPhNull{iSub,curRoi}*1i);
        end
    end

    for iRoi1=1:size(traj,3)
        for iRoi2=1:size(traj,3)
            [procDist(iSub,iRoi1,iRoi2) procTrans{iSub,iRoi1,iRoi2} procTransform{iSub,iRoi1,iRoi2}] = ...
                procrustes(traj{iSub,1,iRoi1},traj{iSub,2,iRoi2});%transforms the 2nd to fit the 1st
        end
    end
end
%%
...
colorbar
if saveFigs
    print('-painters','-dpdf',['~/Documents/MATLAB/rwdRapid/figures/procrustesDistMatrix_10D.pdf']);
end
%%
ifig=ifig+1; figure(ifig); clf
...
if saveFigs
    print('-painters','-dpdf',['~/Documents/MATLAB/rwdRapid/figures/procrustesExamples_10D.pdf']);
end