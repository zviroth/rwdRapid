    ...
    for irun=1:nruns
        for icontrast=1:max(stimContrastTrials{iSub})
            for  ifreq=1:nfreqs
                ...
                trialsInd = trialRunStim{iSub}==irun & stimFreqTrials{iSub}==ifreq & stimContrastTrials{iSub}==icontrast;
    ...
    for iSub=1:length(goodSubs)
        ...
        for iRoi=bensonROIs%1:length(roiNames) %length(bensonROIs)
            for ibin=1:nbins
                ...
                for irun=1:nruns
                    indFirst = (irun-1)*ncontrasts*nfreqs + 1;
                    indLast = irun*ncontrasts*nfreqs;
