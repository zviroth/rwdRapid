...
stimulus.trialLenMin = trialLenMin;
stimulus.trialLenMax = trialLenMax;
...
task{1}{1}.response(1)=0;% = zeros(numTrials,1);
task{1}{1}.correctResponse(1)=0;% = zeros(numTrials,1);
task{1}{1}.correctness(1)=-1;% = zeros(numTrials,1) - 1;
...
while (phaseNum <= length(task)) && ~myscreen.userHitEsc
    % update the task (peripheral gratings)
    [task{2} myscreen phaseNum] = updateTask(task{2},myscreen,phaseNum);
    % update the fixation task (subsidiary)
...
if task.thistrial.thisseg == 1
    stimulus.startSubsidiary = 1;%start the fixation task as well
end
...
stimulus.stimulusSegments = 2:(length(task{2}{1}.segmin) - 1);
...
task.thistrial.numDigits = ceil(fixStimulus.trialLenMax/task.thistrial.SOA);
...
if task.thistrial.thisseg == 1%2%1
    fixStimulus.thisColor = fixStimulus.cueColor;
elseif task.thistrial.thisseg == 3%4%3
...
global stimulus; %used for determining when to begin the trial
global fixStimulus;

% http://gru.stanford.edu/doku.php/mgl/taskreferencehowtos#how_to_run_a_dual_task
...
% if task.thistrial.thisseg == 2 || task.thistrial.thisseg == 4 || task.thistrial.thisseg == 5
    nslices = 32;
