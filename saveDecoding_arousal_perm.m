                    ...
                    for iRoi=bensonROIs%1:length(roiNames) %length(bensonROIs)
                        for ibin=1:nbins
                            ...
                            for arousal=1:2
                                ...
                                classAccFreq{arousalType,featureType,arousal}(iSub,iRoi,ibin) = sum(arousalFreqLabels==classGuessFreq{iSub,iRoi,ibin,arousal})/ntrials;%make sure not to include NaN trials?
                                classAccContrast{arousalType,featureType,arousal}(iSub,iRoi,ibin) = sum(arousalContrastLabels==classGuessContrast{iSub,iRoi,ibin,arousal})/ntrials;%make sure not to include NaN trials?
                                ...
                                for iperm=1:nperms
                                    ...
                                    permAccFreq{arousalType,featureType,arousal}(iSub,iRoi,ibin,iperm) = sum(arousalFreqLabels==permGuessFreq{iSub,iRoi,ibin,arousal}(iperm,:))/ntrials;
