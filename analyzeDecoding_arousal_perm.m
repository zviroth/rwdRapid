...
for arousalType=arousalTypes
    ...
    for iSub=1:length(goodSubs)
        ...
        for arousal=1:2
            ...
            %                 end
            %                 for icontrast=1:max(contrastTrials{goodSubs(iSub),1})
            %                     subBinContrastStdArousal{iSub,iRoi,ibin,icontrast,arousal} =  subBinStdStim{iSub,iRoi,ibin}(:,stimContrastTrials{iSub}==icontrast & arousalTrials{iSub,arousal},:);%vox,trial
            %                 end
        end
        for arousal=1:2
...
%                             arousalData = arousalData(permArousalTrials{iSub,arousal}(iperm,:));
                            ...
                            for itrial=1:ntrials
                                ...
                                trainLabelsContrast = arousalContrastLabels([1:itrial-1 itrial+1:ntrials]);
                                %                                 classGuessContrast{iSub,iRoi,ibin,arousal}(itrial) = classify(testData', trainData', trainLabelsContrast, 'diagLinear');
                                permGuessFreq{iSub,iRoi,ibin,arousal}(iperm,itrial) = classify(testData', trainData', trainLabelsFreq, 'diagLinear');
                                permGuessContrast{iSub,iRoi,ibin,arousal}(iperm,itrial) = classify(testData', trainData', trainLabelsContrast, 'diagLinear');
                                
                            end
                            permAccFreq{featureType,arousal}(iSub,iRoi,ibin,iperm) = sum(arousalFreqLabels==permGuessFreq{iSub,iRoi,ibin,arousal}(iperm,:))/ntrials;
                            permAccContrast{featureType,arousal}(iSub,iRoi,ibin,iperm) = sum(arousalContrastLabels==permGuessContrast{iSub,iRoi,ibin,arousal}(iperm,:))/ntrials;
                        end
                    ...
                    allPermFreqDiff = squeeze(permAccFreq{featureType,1}(iSub,iRoi,ibin,:)) - squeeze(permAccFreq{featureType,2}(iSub,iRoi,ibin,:))';
                    trilOnes = tril(ones(size(allPermFreqDiff)));
                    temp = allPermFreqDiff(trilOnes>0);
                    permAccFreqDiff(featureType,iSub,iRoi,ibin,:) = temp(:);
                    
                    allPermContrastDiff = squeeze(permAccContrast{featureType,1}(iSub,iRoi,ibin,:)) - squeeze(permAccContrast{featureType,2}(iSub,iRoi,ibin,:))';
    ...
    for featureType=featureTypes
        for iRoi=bensonROIs
            for ibin=1:nbins
                [h pvalFreq(featureType,iRoi,ibin)] = ttest(classAccFreq{featureType,1}(:,iRoi,ibin), classAccFreq{featureType,2}(:,iRoi,ibin));
                [h pvalContrast(featureType,iRoi,ibin)] = ttest(classAccContrast{featureType,1}(:,iRoi,ibin), classAccContrast{featureType,2}(:,iRoi,ibin));
                
                permFreqMeanDiff(featureType,iRoi,ibin,:) = mean(permAccFreqDiff(featureType,:,iRoi,ibin,:));
                realDiffFreq(featureType,iRoi,ibin) = mean(classAccFreq{featureType,1}(:,iRoi,ibin) - classAccFreq{featureType,2}(:,iRoi,ibin));
                pvalPermFreq(featureType,iRoi,ibin) = sum(permFreqMeanDiff(featureType,iRoi,ibin,:) >= realDiffFreq(featureType,iRoi,ibin))/length(permFreqMeanDiff(featureType,iRoi,ibin,:));
                
                permContrastMeanDiff(featureType,iRoi,ibin,:) = mean(permAccContrastDiff(featureType,:,iRoi,ibin,:));
                realDiffContrast(featureType,iRoi,ibin) = mean(classAccContrast{featureType,1}(:,iRoi,ibin) - classAccContrast{featureType,2}(:,iRoi,ibin));
                pvalPermContrast(featureType,iRoi,ibin) = sum(permContrastMeanDiff(featureType,iRoi,ibin,:) >= realDiffContrast(featureType,iRoi,ibin))/length(permContrastMeanDiff(featureType,iRoi,ibin,:));
    ...
    for featureType=featureTypes
        ...
        for ibin=1:nbins
            for arousal=1:2
                maxAcc(arousal) = max(squeeze(mean(classAccFreq{featureType,arousal}(:,iRoi,ibin))));
            ...
            if pvalPermFreq(featureType,iRoi,ibin)<pvalThresh
                scatter(ibin,max(maxAcc),markerSize,plotColors{featureType},'s','filled');
            elseif pvalPermFreq(featureType,iRoi,ibin)>1-pvalThresh
    ...
    for featureType=featureTypes
        ...
        for ibin=1:nbins
            for arousal=1:2
                maxAcc(arousal) = max(squeeze(mean(classAccContrast{featureType,arousal}(:,iRoi,ibin))));
                %             maxAcc = max(squeeze(mean(accFreq{featureType,1}(:,iRoi,ibin))),squeeze(mean(accFreq{featureType,2}(:,iRoi,ibin))));
            end
