...
%         controlRoiAmp{iSub,iRoi} = subBinStdNull{iSub,iRoi,nbins};
        controlRoiAmp{iSub,iRoi} = subBinAmpNull{iSub,iRoi,nbins};
...
%         ampPredictors = [controlRoiAmp{iSub,iRoi}];
...
%             [subBinAmpNullCoef(iSub,iRoi,ibin,:),BINT,subBinAmpNullResid{iSub,iRoi,ibin}] = regress(subBinStdNull{iSub,iRoi,ibin},ampPredictors);
            [subBinAmpNullCoef(iSub,iRoi,ibin,:),BINT,subBinAmpNullResid{iSub,iRoi,ibin}] = regress(subBinAmpNull{iSub,iRoi,ibin},ampPredictors);

            [subBinPhNullCoef(iSub,iRoi,ibin,:),BINT,subBinPhNullResid{iSub,iRoi,ibin}] = regress(subBinPhNull{iSub,iRoi,ibin},phPredictors);
            
            
            
            %assume perfect correlation of trial amplitudes between bins.
%             subBinAmpNullCoef(iSub,iRoi,ibin,:) = squeeze(ones(size(subBinAmpNullCoef(iSub,iRoi,ibin,:)))*nanstd(subBinAmpNull{iSub,iRoi,ibin}))./nanstd(ampPredictors)';
%             subBinAmpNullCoef(iSub,iRoi,ibin,:) = subBinAmpNullCoef(iSub,iRoi,ibin,:)*std(subBinAmpNull{iSub,iRoi,ibin})./std(ampPredictors);
            %assume perfect correlation of trial phases between bins.
%             subBinPhNullCoef(iSub,iRoi,ibin,:) = ones(size(subBinPhNullCoef(iSub,iRoi,ibin,:)));
%             subBinPhNullCoef(iSub,iRoi,ibin,:) = squeeze(ones(size(subBinPhNullCoef(iSub,iRoi,ibin,:)))*nanstd(subBinPhNull{iSub,iRoi,ibin}))./nanstd(phPredictors)';
            ...
            %predict response to be mean response X amplitude, shifted by phase
...
%             phShifts{iSub,iRoi,ibin} = round((phPredictors*squeeze(subBinPhNullCoef(iSub,iRoi,ibin,:) )*trialLength/(2*pi)));
            phShifts{iSub,iRoi,ibin} = phPredictors*squeeze(subBinPhNullCoef(iSub,iRoi,ibin,:));
            %use circ_corrcc instead of linear regression?
            phaseDiff = controlRoiMeanPh(iSub,iRoi) - subBinPh(iSub,iRoi,ibin);
            phShifts{iSub,iRoi,ibin} = mod(phShifts{iSub,iRoi,ibin} + pi- phaseDiff,2*pi)-pi;
            phShifts{iSub,iRoi,ibin}(isnan(phShifts{iSub,iRoi,ibin})) = zeros;
            for itrial=1:size(subBinNullAmpEst{iSub,iRoi,ibin},1)
                subBinNullEst{iSub,iRoi,ibin}(itrial,:) = circshift(subBinNullAmpEst{iSub,iRoi,ibin}(itrial,:),-round(phShifts{iSub,iRoi,ibin}(itrial)*trialLength/(2*pi)));%Fourier phase inverse of circshift
            end
 
            %Alternatively, predict response to be sine wave X amplitude, shifted by phase
            f = zeros(length(amps{iSub,iRoi,ibin}),trialLength);
            f(:,2) =  amps{iSub,iRoi,ibin}.*exp(phShifts{iSub,iRoi,ibin}*1i);
            f(:,end) =  amps{iSub,iRoi,ibin}.*exp(-phShifts{iSub,iRoi,ibin}*1i);
            subBinNullFftEst{iSub,iRoi,ibin} = ifft(f,[],2);
...
%             subBinNullEst{iSub,iRoi,ibin} = predictors*squeeze(subBinAmpNullCoef(iSub,iRoi,ibin,:)).*squeeze(subBinMeanResp(iSub,iRoi,ibin,:))';
            ...
            subBinNullFftResid{iSub,iRoi,ibin} = subBinNullTrialTseries{iSub,iRoi,ibin} - subBinNullFftEst{iSub,iRoi,ibin}';
            ...
            fftResidVar(iSub,iRoi,ibin) = nanvar(subBinNullFftResid{iSub,iRoi,ibin}(:));%some trials are NaNs!
        end
    end
...
%    plot(subBinNullResid{iSub,iRoi,ibin}(:,plotTrials));
   subplot(rows,cols,ibin+cols)
   plot(subBinNullFftEst{iSub,iRoi,ibin}(plotTrials,:)');
   subplot(rows,cols,ibin+2*cols)
   plot(subBinNullFftResid{iSub,iRoi,ibin}(:,plotTrials));
...
%%
mean([origVar(:) residVar(:) ampResidVar(:) fftResidVar(:)])
controlOrigVar = origVar(:,:,end); controlResidVar = residVar(:,:,end); controlAmpResidVar = ampResidVar(:,:,end); controlFftResidVar = fftResidVar(:,:,end);
mean([controlOrigVar(:) controlResidVar(:) controlAmpResidVar(:) controlFftResidVar(:)])
...
for ibin=1:nbins
    subplot(rows,cols,ibin)
    plot(hist([subBinPhNull{iSub,iRoi,ibin} phShifts{iSub,iRoi,ibin}]));
    subplot(rows,cols,ibin+cols)
    plot(hist([subBinAmpNull{iSub,iRoi,ibin} amps{iSub,iRoi,ibin}]));
end
%%
