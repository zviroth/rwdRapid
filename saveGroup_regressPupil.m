...
subFolders = {'009020190515', '009020190325/', '009720190603', '009720190531', '008020190509','008020190411', '001920190515','001920190403/', '007420190529', '007420190221/', '008320190523', '008320190320/', '008720190503', '008620190410', '005520190411', '007520190318/','007520190715','010420190710'};
subFolders = {'009020190515',  '009720190603',  '001920190515', '007420190529', '008320190523',  '008720190503', '008620190410', '005520190411', '007520190715/', '008020190509'};

subFolders = {'008320190523',  '008720190503'};
subFolders = {'008620190410', '005520190411', '007520190715/', '008020190509'};
subFolders = {'010420190805','009020190515','009720190603','001920190515', '007420190529', '008320190523',  '008720190503', '008620190410', '005520190411', '007520190715/', '008020190509'};
...
% subFolders = {'001920190515', '007420190529'};
...
for iSub = 1:numSubs
    ...
    for iScan = 1:nScans%2 concatenations, 1 for each reward type
        ...
        for r=1:length(s)
            concatRwdTypeNum(iSub, rwd) = s{r}.myscreen.stimulus.rewardType;
        ...
        nullPupilTrials{iSub,rwd} = NaN(numTrials(rwd),1);
        trialCounter=0;
        for r=1:length(s)         
            rwdPupil{iSub,rwd}(trialCounter+1:trialCounter+runEyeSize{rwd}(r,1), 1:runEyeSize{rwd}(r,2)) = e{r}.eye.pupil;
            nullPupilTrials{iSub,rwd}(trialCounter+1:trialCounter+runEyeSize{rwd}(r,1)) = nullTrialsRun{iSub,rwd,r};
            trialCounter = trialCounter + runEyeSize{rwd}(r,1);
        end
        %plot mean pupil size
        meanPupil{iSub,rwd} = nanmean(rwdPupil{iSub,rwd})';
        meanNullPupil{iSub,rwd} = nanmean(rwdPupil{iSub,rwd}(nullPupilTrials{iSub,rwd}==1,:))';
        meanStimPupil{iSub,rwd} = nanmean(rwdPupil{iSub,rwd}(nullPupilTrials{iSub,rwd}==0,:))';
        ...
        stdPupil{iSub,rwd} = nanstd(rwdPupil{iSub,rwd})';%pupil trial amplitudes
    end
...
%         d{iscan} = viewGet(v, 'd', iscan);
        ...
        numTrials(iSub,rwd) = length(concatInfo{iSub,rwd}.whichScan);
...
%         clear contrastMat sfMat contrastSfMat nullTrialMat
        ...
        for r=1:numRuns(iSub,rwd)%create design matrices per run
            runRwd{iSub,iscan}(r) = s{r}.myscreen.stimulus.rewardType;
...
%             deconvMatContrastFreqRun(end+1,:) = ones;
    ...
    for iRoi=1:numRois
        for ibin=1:nbins
            binHrf(iSub,iRoi,ibin,:) = (numTrials(iSub,1)*binTrialTC(iSub,iRoi,ibin,1,:) + numTrials(iSub,2)*binTrialTC(iSub,iRoi,ibin,2,:))./(sum(numTrials(iSub,:)));
    ...
    for rwd=1:2
        for iRoi=1:numRois
            %create design matrices for each eccentricity bin, using mean response instead of canonical hrf
            for ibin=1:nbins
...
%                     designMatNullBinRun(2+1,:) = ones;
                ...
                %first, deconvolve task-related response scaled by puil size
                trPupilSmallMat = stdPupil{iSub,rwd}.*ones(1,numRuns(iSub,rwd)*trialsPerRun);
                trPupilMat = upsample(trPupilSmallMat,trialLength);%add zeros in between
                deconvPupilMat = zeros(deconvLength,size(trPupilMat,2));
                for j=1:deconvLength
                    deconvPupilMat(j,:) = [zeros(1,j-1) trPupilMat(1,1:end-j+1)];%shift and pad with zeros
                end
                deconvTaskResp(iSub,rwd,:)= deconvPupilMat'\tc;
                tc = tc - (squeeze(deconvTaskResp(iSub,rwd,:))'*deconvPupilMat)';
...
stimulus.freqs = s{1}.myscreen.stimulus.freqs;
stimulus.contrasts = s{1}.myscreen.stimulus.contrasts;
...
save([dataFolder 'rwdRapidData_regressPupil_' nullTrialsString globalMeanString '.mat'], 'dataFolder', 'subFolders', 'samplerate', 'roiNames', ...
